...
function makeResponse(rq, status, reason, extension) {
    ...
    if (extension) {
        if (extension.headers)
            Object.keys(extension.headers).forEach(function (h) { rs.headers[h] = extension.headers[h]; });
...
function makeStreamTransport(protocol, maxBytesHeaders, maxContentLength, connect, createServer, callback) {
    ...
    function init(stream, remote) {
        var remoteid = [remote.address, remote.port].join(), flowid = undefined, refs = 0;
        ...
        stream.on('timeout', function () { if (refs === 0)
            stream.destroy(); });
...
function makeTlsTransport(options, callback) {
    return makeStreamTransport('TLS', options.maxBytesHeaders, options.maxContentLength, function (port, host, callback) { return tls.connect(port, host, options.tls, callback); }, function (callback) {
...
function makeTcpTransport(options, callback) {
    return makeStreamTransport('TCP', options.maxBytesHeaders, options.maxContentLength, function (port, host, callback) { return net.connect(port, host, callback); }, function (callback) {
...
function makeWsTransport(options, callback) {
    ...
    function init(ws) {
        var remote = { address: ws._socket.remoteAddress, port: ws._socket.remotePort }, local = { address: ws._socket.address().address, port: ws._socket.address().port }, flowid = [remote.address, remote.port, local.address, local.port].join();
        flows[flowid] = ws;
    ...
    function makeClient(uri) {
        if (clients[uri])
            return clients[uri]();
        var socket = new WebSocket(uri, 'sip', { procotol: 'sip' }), queue = [], refs = 0;
...
function makeTransactionLayer(options, transport) {
    ...
    return {
        createServerTransaction: function (rq, cn) {
            ...
            return server_transactions[id] = (rq.method === 'INVITE' ? createInviteServerTransaction : createServerTransaction)(cn.send.bind(cn), function () {
